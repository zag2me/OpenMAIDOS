...
ConnectOnce();
include('header.php');
echo "<table width=100% id='admininfos'>";
...
#	findFiles($failed_plugins,$ftp_repository."*.mopp.fail");
...
#	$new_plugins = array();
#	findFiles($new_plugins,$ftp_repository."*.mopp");
#	findFiles($new_plugins,$ftp_manual_uploads."*.mopp");

#	$passed_plugins = array();
#	findFiles($passed_plugins,$ftp_repository."*.mopp.pass");

#	$ignored_plugins = array();
#	findFiles($ignored_plugins,$ftp_repository."*.mopp.ignore");
...
function printCheckForProblems($func, $confirm)
{
	...
	if ($func && $confirm == FALSE) {
		...
		if ($func == "delmpp" && $confirm) {
			$data_files = array();
			$data_files = file_list("mopp/", "*", 1, TRUE);
			$data_files_count = count($data_files);
			if ($data_files_count > 0 )
			{
				printLine("");
	...
	if (!$func) {
		...
		$mpp_files = array();
		$mpp_files = file_list("mopp/", "*", 1, TRUE);
		...
		else printLine("MOPP directory appears to be OK...");
