...
function VoteWorking($profil_id, $plugin_id, $plugin_version, $vote, $uniqueID)
{
	...
	if ($vote == "")
	{
		...
		//header
    //vote system
     echo "<table width='100%' id=votesystem cellpadding='0' cellspacing='0'>";
      echo "<tr>";
        echo "<td align='left'>Does this plug-in work?</td>";
        echo "<td width='100'></td>";
        echo "<td align='right'>";
          if ($totalvotes > 0 && $percentyes > 55) echo "Status: <font color='Green'>Working </font>";
      		elseif ($totalvotes > 0 && $percentyes < 45) echo "Status: <font color='Red'>Broken </font>";
      		elseif ($totalvotes < 1) echo "Status: <font color='Green'>Assumed Working </font>";
        echo "</td>";
      echo "</tr>";
      echo "<tr>";
        echo "<td align='left'>";
           if ($profil_id == "") echo "<a href='virtual_auth.php'>Login to vote</a>";
	         else
		      {
        ...
        echo "</td>";
        echo "<td></td>";
          if ($totalvotes > 0 && $percentyes > 55) echo "<td align='right'><img src='images/works.png' width='25' height='25' border='0'></td>";
          elseif ($totalvotes > 0 && $percentyes < 45) echo "<td align='right'><img src='images/not_works.png' width='25' height='25' border='0'></td>";
          elseif ($totalvotes < 1) echo "<td align='right'><img src='images/unknown.gif' width='25' height='25' border='0'></td>";
      echo "</tr>";
     echo "</table>";
	}
	else
...
function ClearImagesPlugin($plugin_id)
{
  ...
  { 
	 ...
	 // delete image
	 unlink ("$plugin_home_directory/$plugin_id/$Image_Name.png");
	 // Delete thumbails
	 unlink ("$plugin_home_directory/$plugin_id/$Image_Name.mini.png");
	 $i++;
  }
...
function AdminAddMpp($file, $ignore_version, $ignore_sanity, $delete_upload, $force_update, $process_multiple, $u)
{
	...
	if (!$temp_directory || !is_dir($temp_directory))
	{
	...
	echo "Searching for .mopd... ";
	$duplicatesFound = FALSE;
	for ($i=0; $i<sizeof($list); $i++) {
		$index_archive_plugin = "";
		for (reset($list[$i]); $key = key($list[$i]); next($list[$i])) {
			if (strlen($list[$i][$key]) > 5) {
				$slm = substr($list[$i][$key],strlen($list[$i][$key])-5);
				$slm2 = $list[$i][$key];
				if (strcasecmp($slm, '.mopd') == 0) {
					if ($index_archive_plugin <> "") {
						$duplicatesFound = TRUE;
					}
					$index_archive_plugin = $i;
					$pluginfile = $temp_directory. "\\" . $list[$i][$key];
					//debug show plugin file
					echo $pluginfile . " ";
					$pluginxmlfilename = $list[$i][$key];
					//echo $pluginxmlfilename;
					break;
				}
			}
		}
	...
	{
		echo "<b>No .mopd file(s) were found.</b><br>\n";
	...
	if ($duplicatesFound)
		echo "passed (however, there are multiple .mopd's. assuming they're all identical...)<br>\n";
	else
		echo "passed.<br>\n";
    //	echo ("Clearing Temporary directory<br>\n");
    //	remove_directory($temp_directory);
    //	mkdir($temp_directory);
	echo "Extracting .mopd..." . $temp_directory;
	...
	echo "passed.<br>\n";
	unset($archive);
	//try to determine the encoding type of the .mopd file or set to UTF-8 if not specified
	$rx = '/<?xml.*encoding=[\'"](.*?)[\'"].*?>/m';
	...
	if (preg_match($rx, $xml_content, $m))
  {
	...
	if ($encoding_unknown) echo "Parsing .mopd (your encoding '$specified_encoding' is not recognized, trying UTF-8 instead)...";
	else echo "Parsing .mopd (using $encoding)...";
	$Data = array();
	$xml_parser = xml_parser_create($encoding);
	xml_set_element_handler($xml_parser, "StartElement", "EndElement");
	xml_set_character_data_handler($xml_parser, "characterData");
	if (!($fp = fopen($pluginfile, "r"))) {
		echo "<b>Error opening .mopd file.</b><br><br>\n";
		@unlink($pluginfile);
	...
	while ($data = @fread($fp, 4096)) {
		if (!@xml_parse($xml_parser, $data, feof($fp))) {
			...
			echo "<b>Error in .mopd file: '$msg' on line $line</b><br>\n";
			$parser_failed = TRUE;
		}
	}
	...
	fclose($fp);
	if ($parser_failed) {
		echo "We don't recognize the encoding type of your .mopd XML!<br>";
		...
		echo "the type within your .mopd file like this:<br><br>";
	...
	echo "passed.<br>\n";
	if ($encoding_unknown) {
		echo "We didn't recognize the encoding type (<b>$specified_encoding</b>) of your .mopd XML!<br>";
		...
		echo "you can specify the type within your .mopd file like this:<br><br>";
	...
	$moduletypes = array("icon", "misc", "hack", "sub", "database");
	...
	if ( ($plugin_type != "theme" && $module_type == "icon") || $plugin_type == "icon") {
		...
		echo ".mopd file to Theme, set the MODULE-TYPE to Icon, and re-upload the set.<br>";
		echo "If you are not uploading an icon set, please check your plugin and module type<br>";
		echo "settings in the .mopd file and try uploading again.</b><br><br>";
		$pluginfile_error = TRUE;
		}
	...
	if ($pluginID_passed != "passed" && !ignore_sanity) {
		...
		echo "upload again. If you feel this error is incorrect, PM your .mopd file to the<br>";
		...
		else 
	{
		...
		$screenshots = array();
		findFiles($screenshots, $temp_directory . sql_regcase("/screenshot.png"));
		findFiles($screenshots, $temp_directory . sql_regcase("/screenshots/1.png"));
		findFiles($screenshots, $temp_directory . sql_regcase("/screenshots/2.png"));
		findFiles($screenshots, $temp_directory . sql_regcase("/screenshots/3.png"));
		findFiles($screenshots, $temp_directory . sql_regcase("/screenshots/4.png"));
		findFiles($screenshots, $temp_directory . sql_regcase("/screenshots/5.png"));
		findFiles($screenshots, $temp_directory . sql_regcase("/screenshots/6.png"));
		findFiles($screenshots, $temp_directory . sql_regcase("/screenshots/1.jpg"));
		findFiles($screenshots, $temp_directory . sql_regcase("/screenshots/2.jpg"));
		findFiles($screenshots, $temp_directory . sql_regcase("/screenshots/3.jpg"));
		findFiles($screenshots, $temp_directory . sql_regcase("/screenshots/4.jpg"));
		findFiles($screenshots, $temp_directory . sql_regcase("/screenshots/5.jpg"));
		findFiles($screenshots, $temp_directory . sql_regcase("/screenshots/6.jpg"));
#		echo "<br>"; echo "sql_regcase: " . $temp_directory . sql_regcase("/screenshot.png") . "<br>screenshots array:"; print_r($screenshots); echo "<br>";
		...
		foreach($screenshots as $screenshot)
		{
			...
			$newScreenshot = "$plugin_dest_directory/$i.png";
...
//	mail("skypichat@meedios-france.com", "AddMpp", "$u uploaded $plugin_Name version $plugin_Version id $plugin_id for profile $profil_id");
//	return true;
}
...
function TestPluginFile($pluginfile, $u)
{
	...
	$uploaded_file_cnt = count($uploaded_file_tmp) - 1;
	$uploaded_file = $uploaded_file_tmp[$uploaded_file_cnt];
	echo "Checking .mopd file: $uploaded_file<br>\n";
	echo "Checking uploaded file exists... ";
	if (!is_file($pluginfile))
	{
		echo "<b>File doesn't exist!</b><br><br>\n";
	...
	while ($data = @fread($fp, 4096)) {
		if (!@xml_parse($xml_parser, $data, feof($fp))) {
			$msg = xml_error_string(xml_get_error_code($xml_parser));
			$line = xml_get_current_line_number($xml_parser);
			echo "<b>Error in .mopd file: '$msg' line $line</b><br>\n";
			$parser_failed = TRUE;
		}
	}
	xml_parser_free($xml_parser);
	...
	echo "Checking for valid module type...$module_type...";
	$moduletypes = array("icon", "misc", "hack", "web", "wizard", "sub", "database");
	$valid_module_type = IsPluginTypeValid($module_type, $moduletypes);
	...
	if ($pluginID_passed != "passed") {
		...
		echo "the upload again to continue testing this .mopd file.  If you feel this error<br>";
		echo "is incorrect, PM your .mopd file to the OpenMAID admin and ask why it is failing<br>";
		echo "the Plugin ID (GUID) validation check.<br><br>\n";
		echo ("Removing .mopd upload...");
		@unlink($pluginfile);
		echo "Done!<br><br>\n";
		return FALSE;
		}
...
function xtern_summary($gen_cache){
	...
	$summary_xml_file = "xml/summary.xml";
	...
	if ($gen_cache || $send_gen_cache) {
		...
		$sql = "SELECT plugin_ID, max(plugin_VersionValue) as max_VersionValue, max(plugin_Date) as max_Date FROM plugins WHERE plugin_Type='Module' OR plugin_Type='Theme' OR plugin_Type='Import' OR plugin_Type='Service' OR plugin_Type='Input' OR plugin_Type='General' OR plugin_Type='Icon' OR plugin_Type='Misc' AND plugin_ReviewFlag='FALSE' GROUP BY plugin_ID";
    ...
    if (file_exists($summary_cache_file)) unlink($summary_cache_file);
		...
		copy ($summary_cache_file, $summary_xml_file);
...
function xtern_extended($gen_cache){
	...
	$extended_xml_file = "xml/extended.xml";
	$send_gen_cache = FALSE;
	//when gen_cache = FALSE, check for cache file.  If file exists, echo it's contents and exit.
	//If file doesn't exist, set gen_cache = TRUE and continue to generate the cache file and echo
	//it's contents.
	...
	if ($gen_cache || $send_gen_cache) {
		...
		foreach ($state as $stateValue){
			...
			$sql .= "WHERE (plugin_Type='Module' OR plugin_Type='Theme' OR plugin_Type='Service' OR plugin_Type='Import' OR plugin_Type='Input' OR plugin_Type='General' OR plugin_Type='Icon' OR plugin_Type='Misc') AND plugin_State='$stateValue' AND plugin_ReviewFlag='FALSE' ";
    ...
    if (file_exists($extended_cache_file)) unlink($extended_cache_file);
		...
		copy ($extended_cache_file, $extended_xml_file);
...
function xtern_all($gen_cache){
	...
	if ($gen_cache || $send_gen_cache) {
		...
		@mysql_select_db($db_database) or die("Error");
		//$sql = "SELECT * FROM plugins ORDER BY plugin_Name";
		$sql = "SELECT * FROM plugins WHERE plugin_Type='Module' OR plugin_Type='Theme' OR plugin_Type='Service' OR plugin_Type='Import' OR plugin_Type='Input' OR plugin_Type='General' OR plugin_Type='Icon' OR plugin_Type='Misc' AND plugin_ReviewFlag='FALSE' ORDER BY plugin_Name ASC";
		$res = mysql_query($sql);
		while (($enreg=@mysql_fetch_array($res))) {
			$plugin_id = utf8_encode(htmlspecialchars($enreg["plugin_ID"]));
			$plugin_name = utf8_encode(htmlspecialchars($enreg["plugin_Name"]));
    ...
    if (file_exists($all_cache_file)) unlink($all_cache_file);    
		...
		copy ($all_cache_file, $all_xml_file);
...
function xtern_sync($gen_cache){
    ...
    if (file_exists($sync_cache_file)) unlink($sync_cache_file);
		...
		copy ($sync_cache_file, $sync_xml_file);
...
////////////////////////////////////////////////////////////////////////////////
// Return '\\' in Windows, and '/' in Linux
function DirectorySeperator()
{
	if (stristr(php_uname(), 'windows')) { return '\\'; }
	return '/';
}
...
{
        // We're not using PHP's tmpfile function to create a temporary now.
        // As pclzip might not like FQFNs (blame NTFS ACL X attribute). And that's what tempnam() returns.
		// To add more uniqueness to the name, we append the
		// unix timestamp to the $prefix.
		$timestamp = time();
		$prefix = $prefix . $timestamp;
        $tempname = $path . DirectorySeperator() . $prefix;
        if (!$tempname)
		{
			return false;
		}
        if (is_dir($tempname) && !unlink($tempname))
		{
			return false;
		}
...
function getPluginFileName($plugin_Name, $plugin_Version, $plugin_State) {
		...
		$plugin_Name = str_replace("<","[",$plugin_Name);
		$plugin_Name = str_replace(">","]",$plugin_Name);
		return $plugin_Name . "_" . $plugin_Version . "_" . $plugin_State . ".mopp";
}
?>